...
- **Panel view toggle (Ctrl+X T)** — Single/double-column switch now runs in the `ViewModeToggled` action handler (not inline in the chord), so view mode and panel autosave stay consistent.
- **Selected row foreground** — Cursor row styling is centralized in `PanelListPalette::selected_row_style()` (bold dark text on `selected_bg`). Themes use explicit RGB black for `selected_fg` instead of `Color::Black`, so 256-color and 16-color palette adaptation keeps readable highlight text (same class of issue as the F4 gutter fix above).
- **Image gallery (F3) file names** — Multi-file viewer: tab strip width is capped at one fifth of the terminal (image area stays primary); long tab labels use shared middle-ellipsis truncation (`TruncateMode::MiddleEllipsis` in `text_format.rs`). The title row above the footer shows the full path; multi-tab layout reserves space for that chrome. Diff viewer path truncation reuses the same helper.
