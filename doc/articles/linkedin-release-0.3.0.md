...
The headline this time is **markdown in the terminal**. Open a markdown with **F3**, and you get a rendered preview — not a raw dump: headings, lists, tables, fenced code with **syntax highlighting** for common languages, and local embedded images  drawn with the same Kitty / iTerm2 / Sixel / half-blocks path as the image gallery. Browse local docs like a small reader: **Tab** cycles links, **Enter** (or click) follows them, **Backspace** walks the nav stack, and **T** flips back to plain text when you need the source.
